...
# ### 08 Logistic_regression_diabetes
...
# 데이터 불러오기
xy = np.loadtxt('Tensorflow기본_데이터셋/data-03-diabetes.csv',delimiter=',', dtype=np.float32)
xy.shape #(759,9)

# 학습용 데이터 분리 70 %, 531개
x_train = xy[0:531,:-1] # x, 마지막 컬럼을 제외
y_train = xy[0:531,[-1]] # y, 마지막 컬럼만 2차원으로 추출

# 검증용 데이터 분리 30%, 228개
x_test = xy[531:759,:-1] #[531:,:-1]
y_test = xy[531:759,[-1]]
...
# 바이어스는 웨이의 끝과 맞춰줌
# (531,8) * (8,1) = (531,1)
W = tf.Variable(tf.random.normal([8,1]),name='weigh')
